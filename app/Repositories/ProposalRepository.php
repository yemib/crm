...
class ProposalRepository extends BaseRepository
{
    ...
    {
        $contactIds = Contact::where('customer_id', '=', $input['owner_id'])->where('primary_contact', '=',
        ...
        $users = User::whereId($proposal->assigned_user_id)->get();

        if ($proposal->status == Proposal::STATUS_OPEN) {
            if (! empty($input['assigned_user_id'])) {
                foreach ($users as $user) {
                    Notification::create([
                        'title' => 'New Proposal Created',
                        'description' => 'You are assigned to '.$proposal->title,
                        'type' => Proposal::class,
                        'user_id' => $user->id,
                    ]);
                }
            ...
            if (! empty($input['owner_id'])) {
                foreach ($userContacts as $user) {
                    ...
                    foreach ($contactIds as $oldUser) {
                        ...
                        Notification::create([
                            'title' => 'New User Assigned to Proposal',
                            'description' => $user->first_name.' '.$user->last_name.' assigned to '.$proposal->title,
                            'type' => Proposal::class,
                            'user_id' => $oldUser,
                        ]);
                    }
    ...
    public function updateProposal($input, $id)
    {
        ...
        $input['phone'] = removeSpaceFromPhoneNumber($input['phone']);
        $oldUserIds = Proposal::whereId($proposal->id)->get()->pluck('assigned_user_id')->toArray();
        $contactIds = Proposal::whereOwnerId($proposal->owner_id)->pluck('owner_id')->toArray();
        $oldContactIds = Contact::where('customer_id', '=', $proposal->owner_id)->where('primary_contact', '=',
            '1')->pluck('user_id')->toArray();

        $contactId = implode(' ', $contactIds);
        $userId = implode(' ', $oldUserIds);

        $newUserIds = $input['assigned_user_id'];
        $newContactIds = $input['owner_id'];

        $users = User::whereId($newUserIds)->get();
        $contactsIds = Contact::where('customer_id', '=', $input['owner_id'])->where('primary_contact', '=',
            '1')->pluck('user_id')->toArray();
        $userContacts = User::whereIn('id', $contactsIds)->get();
        ...
        if (! empty($oldContactIds) && $newContactIds !== $contactId) {
            foreach ($oldContactIds as $removedUser) {
                Notification::create([
                    'title' => 'Removed From Proposal',
                    'description' => 'You removed from '.$proposal->title,
                    'type' => Proposal::class,
                    'user_id' => $removedUser,
                ]);
            }
