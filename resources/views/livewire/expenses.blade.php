        ...
        <div class="mt-0 mb-3 col-12 d-flex justify-content-end livewire-search">
            @if(!empty($customer))
                <div class="mt-2 m-r-0">
                    {{ Form::select('expense_category',$expenseCategoryArr, $categoryFilter,['id' => 'expenseCategoryCustomer','class' => 'form-control','placeholder' => __('messages.placeholder.select_expanse_category')]) }}
                </div>
            @endif
                    ...
                    <div class="card-body d-flex justify-content-between pt-1 px-3">
                        <div class="d-table w-100">
                            <div>
                                ...
                                <span class="text-decoration-none" data-toggle="tooltip" title="{{ __('messages.common.expense_category') }}"> {{ html_entity_decode($expense->expenseCategory->name) }}
                                </span>
                            </div>
                            ...
                            <big class="d-table-cell w-100">
                                <i class="{{ getCurrencyClassFromIndex($expense->currency) }}"></i> <span
                                        data-toggle="tooltip"
                                        title="{{ __('messages.expense.expense_amount') }}">{{ number_format( $expense->amount, 2) }}</span>
                            </big>
                            </span>
                            <span class="d-table-row w-100">{{Carbon\Carbon::parse($expense->expense_date)->translatedFormat('jS M, Y')}}
                            </span>
                        </div>
                    </div>
                </div>
