                ...
                <div class="row">
                    <div class="form-group col-sm-6">
                        {{ Form::label('company_name', __('messages.customer.company_name').':') }}<span
                                class="required">*</span>
                        {{ Form::text('company_name', null, ['class' => 'form-control','required', 'id'=>'companyName','placeholder'=>__('messages.customer.company_name')]) }}
                    </div>
                    <div class="form-group col-sm-6">
                        {{ Form::label('website', __('messages.customer.website').':') }}
                        {{ Form::url('website', null, ['class' => 'form-control', 'id' => 'convertWebsite','placeholder'=>__('messages.customer.website')]) }}
                    </div>
                    <div class="form-group col-sm-6">
                        {{ Form::label('default_language', __('messages.customer.default_language').':') }}
                        {{ Form::select('default_language', $data['languages'],null, ['id'=>'convertLanguageId','class' => 'form-control','placeholder' => __('messages.placeholder.select_language')]) }}
                    </div>
                    <div class="form-group col-sm-6">
                        {{ Form::label('country', __('messages.customer.country').':') }}
                        {{ Form::select('country', $data['countries'], null, ['id'=>'convertCountryId','class' => 'form-control']) }}
                    </div>
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        {{ Form::text('email', null, ['id' => 'emailId','class' => 'form-control','required','placeholder'=>__('messages.user.email')]) }}
                    </div>
                    <div class="form-group col-sm-6">
                        ...
                        <input type="password" name="password" required class="form-control" autocomplete="off"
                               minLength="6" maxLength="10" placeholder="{{__('messages.user.password')}}">
                    </div>
                </div>
