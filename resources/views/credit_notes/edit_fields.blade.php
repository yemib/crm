...
<div class="card-body">
    ...
    <div class="row">
        ...
        <div class="form-group col-lg-4 col-md-4 col-sm-12">
            ...
            {{ Form::select('customer_id', $data['customers'], isset($creditNote->customer_id) ? $creditNote->customer_id : null, ['class' => 'form-control', 'required', 'id' => 'customerSelectBox', 'placeholder' => __('messages.placeholder.select_customer')]) }}
        </div>
        <div class="form-group col-lg-4 col-md-4 col-sm-12">
            ...
            <div class="input-group">
                ...
                {{ Form::text('credit_note_number', isset($creditNote->credit_note_number) ? $creditNote->credit_note_number : generateUniqueInvoiceNumber(), ['class' => 'form-control', 'required', 'id' => 'creditNoteNumber','placeholder'=> __('messages.credit_note.credit_note_number')]) }}
            </div>
        </div>
        <div class="form-group col-lg-4 col-md-4 col-sm-12">
            ...
            <div class="input-group">
                ...
                {{ Form::text('credit_note_date', isset($creditNote->credit_note_date) ? date('Y-m-d H:i:s', strtotime($creditNote->credit_note_date)) : null, ['class' => 'form-control datepicker', 'required', 'autocomplete' => 'off','placeholder'=>__('messages.credit_note.credit_note_date')]) }}
            </div>
        </div>
        <div class="form-group col-lg-4 col-md-4 col-sm-12">
            ...
            <select id="creditNoteCurrencyId" data-show-content="true" class="form-control currency-select-box"
                ...
                <option value="0" disabled="true" {{ isset($creditNote->currency) ? '' : 'selected' }}>{{ __('messages.placeholder.select_currency') }}
                </option>
            ...
            <table class="table table-responsive-sm table-responsive-md table-striped table-bordered" id="itemTable">
                ...
                <tr>
                    ...
                    <th class="small-column">{{ __('messages.products.rate') }}(<i
                                data-set-currency-class="true"></i>)<span
                                class="required">*</span></th>
                ...
                @foreach($creditNote->salesItems as $item)
                    <tr>
                        <th><input type="text" name="item[]" class="form-control item-name" required
                                   value="{{ html_entity_decode($item->item) }}" placeholder="{{__('messages.invoice.item') }}"></th>
                        <td><textarea name="description[]" class="form-control item-description" placeholder="{{ __('messages.common.description') }}"
                            >{!! nl2br(e($item->description)) !!}</textarea></td>
                        <td><input type="text" name="quantity[]" class="form-control qty" required min="0"
                                   value="{{$item->quantity}}" placeholder="{{ __('messages.invoice.qty')}}"></td>
    ...
    <div class="row">
        <div class="form-group col-lg-12 col-md-12 col-sm-12">
            <div class="btn-group dropup open">
                {{ Form::button(__('messages.common.save'), ['class' => 'btn btn-primary']) }}
