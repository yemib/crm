        ...
        <td>
            <table width="100%">
                <tr>
                    <td>
                        <div>
                            <table class="invoice-date-table" width="100%">
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.contract.start_date').':' }}</td>
                                    <td>{{ !empty($contract->start_date) ? Carbon\Carbon::parse($contract->start_date)->translatedFormat('jS M, Y')                                                        : __('messages.common.n/a')}}</td>
                                </tr>
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.contract.end_date').': ' }}</td>
                                    <td>{{ !empty($contract->end_date) ? Carbon\Carbon::parse($contract->end_date)->translatedFormat('jS M, Y') :                                                              __('messages.common.n/a')}}</td>
                                </tr>
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.contract.customer_id').': ' }}</td>
