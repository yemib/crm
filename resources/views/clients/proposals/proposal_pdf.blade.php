...
<table class="main-table">
    <tr>
        <td class="app-logo">
            <img src="{{ $settings['logo'] }}" width="100px" alt="logo">
        </td>
                ...
                <tr>
                    <td>
                        ...
                        <div class="text-right">
                            <table width="100%" class="invoice-date-table">
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.proposal.proposal_date').': ' }}</td>
                                    <td>{{ !empty($proposal->date) ? Carbon\Carbon::parse($proposal->date)->translatedFormat('jS M, Y') : __('messages.common.n/a') }}</td>
                                </tr>
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.proposal.open_till').': ' }}</td>
                                    <td>{{ !empty($proposal->open_till) ? Carbon\Carbon::parse($proposal->open_till)->translatedFormat('jS M, Y') : __('messages.common.n/a') }}</td>
                                </tr>
                                <tr>
                                    <td class="font-weight-bold">{{ __('messages.proposal.member').': ' }}</td>
        ...
        <td colspan="2">
            <table width="100%" class="table table-bordered invoice-sales-items mt-2">
                ...
                <tr>
                    ...
                    <th scope="col" class="item-col">{{ __('messages.proposal.item') }}</th>
                    <th scope="col" class="text-right item-qty">{{ __('messages.proposal.qty') }}</th>
                    <th scope="col" class="text-right rate">{{ __('messages.proposal.rate') }}(<span
                                class="pdf-css">&#{{ getCurrencyIcon($proposal->currency) }}</span>)
                    </th>
                    <th scope="col" class="item-taxes">{{ __('messages.proposal.taxes') }}</th>
