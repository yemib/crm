...
<div class="row">
    ...
    <div class="form-group col-sm-4">
        ...
        <p><span data-toggle="tooltip" data-placement="right"
                 title="{{ \Carbon\Carbon::parse($contact->created_at)->translatedFormat('jS M, Y') }}">{{ $contact->created_at->diffForHumans() }}</span>
        </p>
    </div>
    <div class="form-group col-sm-4">
        {{ Form::label('updated_at', __('messages.common.last_updated').':') }}
        <p><span data-toggle="tooltip" data-placement="right"
                 title="{{ \Carbon\Carbon::parse($contact->updated_at)->translatedFormat('jS M, Y') }}">{{ $contact->updated_at->diffForHumans() }}</span>
        </p>
    </div>
</div>
<div class=" row">
    <div class="form-group col-sm-4">
        <i class="font-size-medium {{$contact->primary_contact == 1? 'fas fa-check-circle text-success' : 'fas fa-times-circle text-danger'}}"></i>
        &nbsp{{__('messages.contact.primary_contact') }}
    </div>
    <div class="form-group col-sm-4">
        <i class="font-size-medium {{$contact->send_welcome_email == 1? 'fas fa-check-circle text-success' : 'fas fa-times-circle text-danger'}}"></i>
            &nbsp{{__('messages.contact.send_welcome_email') }}
        </div>
    <div class="form-group col-sm-4">
